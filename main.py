...
def send_fear_and_greed():
    ...
    if len(supplied_token) > 256 or not hmac.compare_digest(
        supplied_token.encode("utf-8"), JOB_TOKEN.encode("utf-8")
    ):
    ...
    if not _send_lock.acquire(blocking=False):
        return "send already in progress", 429, {"Retry-After": "60"}
    global _last_send_attempt
    try:
        now = time.monotonic()
        if _last_send_attempt is not None:
            remaining = SEND_COOLDOWN_SECONDS - (now - _last_send_attempt)
            if remaining > 0:
                return "send rate limited", 429, {"Retry-After": str(int(remaining) + 1)}
        # A timeout may occur after delivery; throttle retries as well as successes.
        _last_send_attempt = now
